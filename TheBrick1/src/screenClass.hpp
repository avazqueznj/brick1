...
public:
    ...
    screenClass(settingsClass *settingsParam, ScreensEnum screenIdParam) : settings{settingsParam}, screenId{screenIdParam}
    {
    ...
    void keyListScrolling(String key)
    {
        ...
        // is it a list ?
        if (!list || !lv_obj_check_type(list, &lv_list_class))
            return;
