        ...
        size_t &outLen)
    {
        ...
        try
        {
            ...
            if (httpStatus < 200 || httpStatus >= 300)
            {
                ...
                Serial.print("[FATAL] HTTP error code: ");
                Serial.println(httpStatus);
                String errorMsg;
            ...
            {
                Serial.print("[FATAL] Bad Content-Type: ");
                Serial.println(contentType);
    ...
    void GET(String serverURL, String ssid, String pass, String path, std::vector<String> &response)
    {
        ...
        while (client.connected() || client.available())
        {
            ...
            else
            {
                ...
                delayBlink();
                wait += 100;
                if (wait >= BRICK_HTTP_READ_TIMEOUT)
                    throw std::runtime_error("*** ERROR *** read timeout -");
            }
        }
...
#define POST_JPG_AK_TIMEOUT 10000
        ...
        size_t len)
    {
        ...
        try
        {
            ...
            Serial.println("==============================");
            Serial.print("[UPLOAD] Starting Binary POST ");
            Serial.println(fullPath);
            Serial.print("[UPLOAD] PK: ");
            Serial.println(uuid);
            ...
            while (client.available() == 0)
            {
                if (millis() - timeout > POST_JPG_AK_TIMEOUT)
                { // shorten bcs stupid replit issue with errors
            ...
            {
                Serial.print("[UPLOAD] 200 SUCCESS Final Result: ");
                Serial.println(response);
                return true;
            }
            ...
            {
                Serial.print("[UPLOAD] << 409 DUPLICATED >> ");
                Serial.println(response);
                return true;
            }

            Serial.print("ERROR!!!! Server rejected upload, NO RETRY! ");
            Serial.println(response);
            return true;
        }
