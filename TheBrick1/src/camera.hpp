...
public:
    ...
    void setLastJpegSize(size_t size)
    {
        ...
        {
            Serial.print("[CRITICAL] Oversized JPEG: ");
            Serial.println(size);
...
public:
    ...
    OV767cameraClass() : cameraClass(), ov767x(), cam(ov767x), fb()
    {
        ...
        if (!raw_mem)
            throw std::runtime_error("CRITICAL: SDRAM Malloc failed!");
    ...
    void shoot() override
    {
        ...
        for (int i = 0; i < CALIBRATION_SHOTS; i++)
        {
            ...
            {
                Serial.print("[WARN] Adjustment ERROR at frame ");
                Serial.println(i);
            }
            Serial.print(".");
        }
        ...
        if (status != 0)
        {
            Serial.print("[ERROR] grabFrame Status: ");
    ...
    void renderPicFromPixelsXXXXXXXXX()
    {
        ...
        if (cam_img_obj == NULL)
        {
            ...
            if (!cam_img_obj)
                throw std::runtime_error("LVGL ERROR: Image object creation failed!");
            ...
            lv_obj_add_event_cb(btn_dismiss, [](lv_event_t *e)
                ...
                Serial.println("[UI] View dismissed."); }, LV_EVENT_PRESSED, NULL);
        }
    ...
    size_t encodePixelsToJPG()
    {
        ...
        if (rc != JPEGE_SUCCESS)
        {
            Serial.print("[ERROR] encodeBegin failed: ");
            Serial.println(rc);
            return 0;
        }
...
public:
    ...
    OV2640MiniCameraClass() : cameraClass()
    {
        ...
        pinMode(MINICAM_CS_PIN, OUTPUT);
        digitalWrite(MINICAM_CS_PIN, HIGH);
    ...
    void shoot() override
    {
        ...
        for (uint32_t i = 0; i < len; i++)
            TJPGDECBuffer[i] = SPI.transfer(0x00);
        miniCam->CS_HIGH();
...
private:
    ...
    cameraManagerClass()
    {
        ...
        if (!TJPGDECBuffer)
            throw std::runtime_error("CRITICAL: SDRAM Malloc failed for JPEG Buffer!");
        TJPGDECWorkSpace = (uint8_t *)SDRAM.malloc(32000);
        if (!TJPGDECWorkSpace)
            throw std::runtime_error("CRITICAL: SDRAM Malloc failed for Workspace!");
        // JPEG DECODE ==================
...
public:
    ...
    void displayJpegFromSDRAM(lv_obj_t *jpg_holder)
    {
        ...
        try
        {
            ...
            Serial.print("[PIC] Dimensions: ");
            Serial.print(jw);
            Serial.print("x");
            Serial.println(jh);
            Serial.print("[PIC] Center calculated: x=");
            Serial.print(x);
            Serial.print(" y=");
            Serial.println(y);
...
private:
    ...
    int findBucketIndex(String targetUUID = "")
    {
        ...
        if (!bd)
            throw std::runtime_error("QSPI CRITICAL: BlockDevice is NULL");

        if (bd->init() != 0)
            throw std::runtime_error("QSPI INIT FAIL during scan");
        ...
        if (foundIndex == -1)
        {
            ...
            else
            {
                ...
                Serial.print("[ERROR] UUID NOT FOUND: ");
...
public:
    ...
    String saveJPGSDRAMToWarehouse(String newPK, BucketType type = BT_USER_PIC)
    {
        ...
        Serial.print("[WAREHOUSE] Target Slot: ");
        Serial.println(bucket);
        Serial.print("[WAREHOUSE] Assigning PK: ");
        Serial.println(newPK);

        mbed::BlockDevice *bd = mbed::BlockDevice::get_default_instance();
        bd->init();

        if (bd->erase(targetAddr, PIC_SLOT_SIZE) != 0)
            throw std::runtime_error("QSPI ERASE FAIL");
        if (bd->program((uint8_t *)&header, targetAddr, 64) != 0)
            throw std::runtime_error("QSPI HEADER FAIL");
        if (bd->program(TJPGDECBuffer, targetAddr + 64, header.length) != 0)
            throw std::runtime_error("QSPI DATA FAIL");
    ...
    {
        Serial.println("------------------------------------------");
        Serial.print("[WAREHOUSE] SEARCHING FOR PK: ");
        Serial.println(targetUUID);

        int bucket = findBucketIndex(targetUUID);
        if (bucket == -1)
            throw std::runtime_error("WAREHOUSE EXCEPTION: PK not found!");
        ...
        if (rc != 0)
            throw std::runtime_error("QSPI READ FAIL");
    ...
    {
        Serial.println("------------------------------------------");
        Serial.print("[WAREHOUSE] ZAP REQUESTED FOR PK: ");
        Serial.println(targetUUID);
        ...
        if (bucket == -1)
        {
            Serial.print("[WARN] Zap failed - UUID not found in silicon: ");
            Serial.println(targetUUID);
            return;
        }
        ...
        Serial.print("[WAREHOUSE] Found PK in Slot: ");
        Serial.println(bucket);
        Serial.print("[WAREHOUSE] Nuking Address: 0x");
        Serial.println(targetAddr, HEX);

        mbed::BlockDevice *bd = mbed::BlockDevice::get_default_instance();
        if (bd == NULL)
            throw std::runtime_error("QSPI CRITICAL: BD is NULL");

        int rc = bd->init();
        if (rc != 0)
            throw std::runtime_error("QSPI ZAP: Init failed");
    ...
    void zapAllWarehouseSlots()
    {
        ...
        int rc = bd->init();
        if (rc != 0)
            throw std::runtime_error("QSPI GLOBAL ZAP: Init failed");
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            if (header.length != 0 && header.length != 0xFFFFFFFF)
            {
                Serial.print("FOUND DATA. PK: ");
                Serial.print(header.uuid);
                Serial.print(" (");
    ...
    std::vector<WarehouseItem> getWarehouseInventory(uint32_t filterType = 0)
    {
        ...
        if (filterType == 0)
            ...
            Serial.println(filterType == BT_LAYOUT_PIC ? "LAYOUTS ONLY" : "USER PICS ONLY");
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            Serial.print("[SLOT ");
            if (i < 10)
                Serial.print("0"); // Keep columns aligned
            Serial.print(i);
            Serial.print("] ");
            ...
            if (header.length != 0 && header.length != 0xFFFFFFFF)
            {
                ...
                {
                    Serial.print("!! GARBAGE !! Length: ");
                    Serial.print(header.length);
                    Serial.print(" PK: ");
                    Serial.println(header.uuid);
                }
        ...
        bd->deinit();
        Serial.print("[WAREHOUSE] Inventory Scan Complete. Found: ");
        ...
        try
        {
            ...
            for (auto &item : pending)
            {
                try
                {
                    ...
                    {
                        Serial.print("[JANITOR] Skipping corrupt slot: ");
                        Serial.println(item.uuid);
                        continue;
                    }
                    ...
                    Serial.print("[JANITOR] ACK Received for: ");
                    Serial.println(item.uuid);
                ...
                catch (const std::exception &e)
                {
                    ...
                    Serial.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                    Serial.print("[JANITOR] SYNC FAILED: ");
                    Serial.println(e.what());
                    Serial.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                    break;
                }
