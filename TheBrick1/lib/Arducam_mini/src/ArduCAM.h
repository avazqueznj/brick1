...
// PROGMEM helpers: on mbed targets flash is memory-mapped, so plain pointers work.
#ifndef PROGMEM
#define PROGMEM
#endif
...
// -----------------------------------------------------------------------------
// Arduino Mbed OS cores (e.g. Arduino GIGA R1 WiFi - STM32H747, mbed_giga)
//
// This library historically used direct port register operations for speed on
// AVR/SAM/STM32 (Arduino_Core_STM32). The Arduino mbed cores do not expose the
// same port register APIs (portOutputRegister/digitalPinToPort/etc.) in a
// compatible way. For mbed targets we fall back to digitalWrite-based CS
// toggling (same approach used for ESP/NRF in this file).
// -----------------------------------------------------------------------------
#if defined(ARDUINO_ARCH_MBED) || defined(ARDUINO_ARCH_MBED_GIGA) || defined(ARDUINO_GIGA)
...
// -----------------------------------------------------------------------------
// Generic PROGMEM helpers (covers Arduino mbed cores such as Arduino GIGA).
// Many modern cores map flash into the normal address space, so plain pointer
// dereference is sufficient. Define these only if the core hasn't already.
// -----------------------------------------------------------------------------
#ifndef PROGMEM
#define PROGMEM
#endif
