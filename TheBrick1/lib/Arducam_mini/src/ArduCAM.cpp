...
ArduCAM::ArduCAM(byte model ,int CS)
{
	...
	#else
		// Arduino mbed cores (e.g. GIGA R1 WiFi) do not provide the classic
		// portOutputRegister/digitalPinToPort fast GPIO APIs used below.
		// Use the same CS handling strategy as ESP/NRF: store the pin number in
		// B_CS and toggle it via digitalWrite (see cbi/sbi macros in ArduCAM.h).
		#if (defined(ESP8266)||defined(ESP32)||defined(TEENSYDUINO) ||defined(NRF52840_XXAA) || defined(ARDUINO_ARCH_MBED) || defined(ARDUINO_ARCH_MBED_GIGA) || defined(ARDUINO_GIGA))
		  B_CS = CS;
		#else
